/* =========================
   PAY54 Dashboard — Layer 2 Wiring (v8.0 aligned)
   Full replacement for assets/js/dashboard.js

   Covers:
   - Currency switching updates balance
   - Theme toggle persists
   - Profile dropdown fully clickable + logout
   - Money Moves / Services / Shortcuts wired (no dead buttons)
   - Requests & Alerts populated + Clear All works
   - Recent Transactions populated
   - Receipts generated for actions
   - View All opens Ledger modal

   Note:
   - This is mock Layer 2 logic (no backend).
...
(() => {
  ...
  // Default demo balances (used if nothing stored yet)
  ...
  // Cross-border FX (You send) currencies (as you approved earlier)
  ...
  function getTx() {
    return safeJSONParse(localStorage.getItem(LS.TX), []);
  }

  function setTx(list) {
    localStorage.setItem(LS.TX, JSON.stringify(list));
  }

  function getAlerts() {
    return safeJSONParse(localStorage.getItem(LS.ALERTS), []);
  }

  function setAlerts(list) {
    localStorage.setItem(LS.ALERTS, JSON.stringify(list));
  }
  ...
  function ensureModalStyles() {
    ...
    style.textContent = `
      ...
      .p54-receipt b{ font-weight: 900; }
      .p54-receipt .muted{ opacity: .75; font-size: 12px; margin-top: 2px; }
      ...
      /* Mobile-only: timestamp under purpose look */
      @media (max-width: 980px){
        .feed-item .feed-main{ gap: 2px; }
        .feed-item .feed-sub{ margin-top: 2px; }
      }
    `;
  ...
  function openModal({ title, bodyHTML, onMount }) {
    ...
    if (typeof onMount === "function") onMount({ backdrop, modal, close });
  ...
  // Profile name/email (use stored if available)
  ...
  // Avatar initial FIX (your HTML uses .avatar-btn, not .profile-btn)
  ...
  // Profile menu click FIX: your HTML uses data-nav, not data-action
  document.querySelectorAll(".pm-item[data-nav]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      openModal({
        ...
        `,
        onMount: ({ modal, close }) => {
          modal.querySelector("#okBtn").addEventListener("click", close);
        }
      });
    });
  });
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener("click", () => {
      // Spec: clears session and redirects to login (keep balances/tx for demo)
  ...
  /* ---------------------------
     4) UI: Alerts + Recent Tx rendering
  --------------------------- */
        ...
        openModal({
          ...
          bodyHTML: `
            ...
            <div class="p54-actions">
              <button class="p54-btn" type="button" id="closeA">Close</button>
            </div>
          `,
  ...
  // We’ll render recent tx by updating the existing feed in DOM (first card in right column)
  function prependTxToDOM(tx) {
    const txCard = document.querySelector(".right .card .feed"); // first feed on right column = recent tx
    if (!txCard) return;
    ...
    item.innerHTML = `
      ...
      <div class="feed-amt ${amtClass}">${sign} ${moneyFmt(tx.currency, Math.abs(tx.amount)).replace(symbols[tx.currency] ? `${symbols[tx.currency]} ` : "", `${symbols[tx.currency]}`)}</div>
    `;

    txCard.prepend(item);

    // keep only 5 on dashboard
    const items = txCard.querySelectorAll(".feed-item");
  ...
  function openLedger() {
    ...
    openModal({
      ...
      bodyHTML: `
        <div class="p54-note">Dashboard shows the latest 5. “View all” opens the ledger (recommended pattern).</div>
        <div class="p54-divider"></div>
  ...
  function openSendPay54() {
    ...
    openModal({
      ...
      onMount: ({ modal, close }) => {
        ...
        const cancel = modal.querySelector("#cancelSend");

        function calcFee(a) {
          // Mock fee: 0.8% capped (demo)
          const n = Number(a || 0);
  ...
  function openReceive() {
    openModal({
      ...
      bodyHTML: `
        <div class="p54-receipt">
          <b>Receive money</b>
          <div class="muted">Share your account, PAY54 tag & QR.</div>
          <div class="p54-divider"></div>
    ...
    openModal({
      ...
      onMount: ({ modal, close }) => {
        ...
        function mockRate(s, r) {
          // intentionally simple demo rates
          const table = {
        ...
        modal.querySelector("#fxForm").addEventListener("submit", (e) => {
          ...
          // If we don't have that wallet yet, treat as 0
  ...
  function openShopOnTheFly() {
    openModal({
      ...
      bodyHTML: `
        ...
        <div class="p54-divider"></div>
        <div class="p54-actions" style="justify-content:flex-start; flex-wrap:wrap;">
    ...
    openModal({
      ...
      bodyHTML: `
        <div class="p54-note">
          ...
          - Add card
          <br><br>
          <b>Camera Scan (approved):</b> “Scan card with camera to autofill details” will be implemented in Layer 3 with device APIs.
        </div>
      ...
      onMount: ({ modal, close }) => {
        ...
        modal.querySelector("#addCardBtn").addEventListener("click", () => {
          alert("Add card form (Layer 3: tokenised + issuer flows).");
        });

        modal.querySelector("#scanBtn").addEventListener("click", () => {
          alert("Camera scan is approved — will be wired in Layer 3 using device camera APIs.");
        });
    ...
    openModal({
      ...
      bodyHTML: `
        <form class="p54-form" id="agentForm">
          <div class="p54-note">
            Agent application requires <b>selfie</b> (approved).
          </div>
          <div>
  ...
  /* ---------------------------
     8) Wire Buttons in dashboard.html
  --------------------------- */
  ...
  document.querySelectorAll(".tile-btn[data-service]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // Remaining services: placeholders (Layer 2 present; deeper logic in Layer 3)
      const names = {
      ...
      openModal({
        title: names[s] || "Service",
        bodyHTML: `
          <div class="p54-note">
            This screen is wired (no dead buttons). Full flows + receipts for this service will be expanded next.
          </div>
          <div class="p54-actions"><button class="p54-btn primary" type="button" id="okSvc">OK</button></div>
        `,
    ...
    btn.addEventListener("click", () => {
      ...
      if (s === "savings") {
        return openModal({
          ...
          bodyHTML: `
            <div class="p54-note">Savings pot creation will be fully expanded next.</div>
            <div class="p54-actions"><button class="p54-btn primary" type="button" id="okSav">OK</button></div>
          `,
          onMount: ({ modal, close }) => modal.querySelector("#okSav").addEventListener("click", close)
        });
      }
      if (s === "invest") {
        return openModal({
          ...
          bodyHTML: `
            <div class="p54-note">Mock assets + portfolio view will be expanded next.</div>
            <div class="p54-actions"><button class="p54-btn primary" type="button" id="okInv">OK</button></div>
          `,
          onMount: ({ modal, close }) => modal.querySelector("#okInv").addEventListener("click", close)
        });
      }
    });
  });
