/* =========================
   PAY54 Dashboard — Layer 2 Wiring (v8.0 aligned) — v805.1
   Full replacement for assets/js/dashboard.js

   Fixes v805.1:
   ✅ Mobile + Desktop Recent Transactions updates correctly (VISIBLE feed only)
   ✅ “View all” wiring fixed (viewAllTx + viewAllTxMobile)
   ✅ Currency stored value validated (fallback to NGN)
   ✅ Default theme = LIGHT (toggle still works)
   ✅ Hardening: no dead buttons, safe localStorage parsing
========================= */
...
(() => {
  ...
  function ensureModalStyles() {
    ...
    style.textContent = `
      ...
      .p54-input,.p54-select{ height:44px; border-radius:12px; border:1px solid rgba(255,255,255,.14); background:rgba(255,255,255,.04); color:inherit; padding:0 12px; outline:none; }
      ...
      .p54-btn{ height:40px; border-radius:999px; border:1px solid rgba(255,255,255,.16); background:rgba(255,255,255,.05); color:inherit; padding:0 14px; font-weight:900; cursor:pointer; }
      body.light .p54-btn{ border-color:rgba(10,20,40,.14); background:rgba(10,20,40,.04); }
  ...
  // ✅ KEY FIX: Always target the VISIBLE RecentTx feed (desktop OR mobile)
  function getVisibleRecentTxFeed() {
    const feeds = Array.from(document.querySelectorAll('.feed[data-role="recentTxFeed"]'));
    if (!feeds.length) return null;

    // offsetParent === null generally means it's display:none or not in layout
    const visible = feeds.find(f => f.offsetParent !== null);
    return visible || feeds[0];
  }

  function prependTxToDOM(tx) {
    const txFeed = getVisibleRecentTxFeed();
    if (!txFeed) return;
