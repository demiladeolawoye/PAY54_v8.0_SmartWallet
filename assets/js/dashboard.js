...
function openCheckout(){
  ...
  openModal({
    ...
    bodyHTML:`
      <form class="p54-form" id="checkoutForm">

        <div class="p54-note">Merchant Payment</div>

        <div>
          <div class="p54-label">Merchant</div>
          <input class="p54-input" id="coMerchant" value="Amazon Store" readonly>
        </div>

        <div>
          <div class="p54-label">Amount</div>
          <input class="p54-input" id="coAmount" value="5000" readonly>
        </div>

        <div>
          <div class="p54-label">Currency</div>
          <input class="p54-input" id="coCurrency" value="${getSelectedCurrency()}" readonly>
        </div>

        <div class="p54-actions">
          <button class="p54-btn" type="button" id="cancelCO">Cancel</button>
          <button class="p54-btn primary">Pay Now</button>
        </div>
    ...
    onMount: ({modal, close}) => {
      ...
      modal.querySelector("#cancelCO").addEventListener("click", close);

      form.addEventListener("submit", (e) => {

        e.preventDefault();

        const merchant = modal.querySelector("#coMerchant").value;
        const amount = Number(modal.querySelector("#coAmount").value);
        const currency = modal.querySelector("#coCurrency").value;
        ...
        requestPinVerification(() => {
          ...
          if(funding.source === "wallet"){
            ...
            if((funding.card.balance || 0) < amount){
              alert("Card insufficient");
              return;
            }

            funding.card.balance -= amount;

            localStorage.setItem("pay54_cards", JSON.stringify({
              list: JSON.parse(localStorage.getItem("pay54_cards")).list
            }));
